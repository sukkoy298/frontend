  ...
  return (
    <div className="w-full max-w-md font-sans">
      ...
      <div className={`bg-zinc-900 border border-zinc-800 rounded-2xl p-8 text-center mb-6 relative overflow-hidden ${loading ? 'opacity-50' : ''}`}>
        ...
        <p className="font-mono text-5xl font-bold text-green-500 leading-none">
          {loading ? '...' : price}
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6">
        ...
        <div className="mb-4">
          <label className="block text-gray-500 text-xs uppercase tracking-wide mb-2">Monto en dólares (USD)</label>
          <div className="relative">
            <input
              ...
              placeholder="0.00"
              className="w-full bg-zinc-950 border border-zinc-800 rounded-lg py-3.5 px-4 text-lg text-white font-mono outline-none focus:border-green-500 focus:ring-1 focus:ring-green-500/30 transition-all"
            />
            <span className="absolute right-4 text-gray-500 font-mono text-sm top-1/2 -translate-y-1/2">USD</span>
          </div>
        </div>
        ...
        <button
          onClick={fetchPrice}
          className="w-full mt-4 py-3 bg-transparent border border-zinc-800 rounded-lg text-gray-400 text-sm font-mono flex items-center justify-center gap-2 hover:border-green-500 hover:text-green-500 transition-all cursor-pointer"
        >
